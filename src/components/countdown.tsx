...
export function Countdown({ targetDate, closestGuess, expectedDate, isActualDate = false }: CountdownProps) {
  ...
  useEffect(() => {
    ...
    const calculateTimeLeft = () => {
      ...
      if (expectedDate) {
        const expected = expectedDate instanceof Date 
          ? expectedDate 
          : new Date(expectedDate.seconds * 1000);
          
        const totalDuration = expected.getTime() - new Date(expected.getTime() - 30 * 24 * 60 * 60 * 1000).getTime();
        const elapsed = expected.getTime() - now.getTime();
        const calculatedProgress = Math.min(100, Math.max(0, 100 - (elapsed / totalDuration) * 100));
      ...
      <CardContent className="space-y-6">
        ...
        {closestGuess && (
          <div className="mt-6 text-center">
            <p className="text-sm text-muted-foreground">Palpite mais próximo</p>
            <p className="font-semibold">{closestGuess.userName}</p>
            <p className="text-sm">
              {new Date(closestGuess.guessDate.seconds * 1000).toLocaleDateString('pt-BR', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
